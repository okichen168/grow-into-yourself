...
export default function SelfCheck() {
  ...
  return <section className="self-check" id="self-check">
    ...
    <div className="check-tabs" role="tablist" aria-label="Relationship type">{(Object.keys(questionnaires) as Kind[]).map((item) => <button type="button" role="tab" aria-selected={kind === item} className={kind === item ? "active" : ""} onClick={() => chooseKind(item)} key={item}>{questionnaires[item].label}</button>)}</div>
    <p className="check-intro">{config.intro}</p>
    <div className="question-list">{config.questions.map((question, index) => <fieldset className="question-card" key={question.text}><legend><span>{String(index + 1).padStart(2, "0")}</span>{question.text}</legend><div className="frequency-row">{scale.map((label, value) => <label key={label}><input type="radio" name={`question-${index}`} checked={answers[index] === value} onChange={() => { const next = [...answers]; next[index] = value; setAnswers(next); setShowResults(false); }} /><span>{label}</span></label>)}</div></fieldset>)}</div>
