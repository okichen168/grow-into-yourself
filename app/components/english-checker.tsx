...
const copy = {
  en: {
    ...
    unavailable: "Deep analysis is temporarily unavailable. Please try again later.", basic: "Basic mode", retry: "Try again",
    clear: "Clear this conversation", confirm: "Clear this input and its analysis?", cancel: "Cancel", confirmClear: "Clear",
  },
  zh: {
    ...
    unavailable: "深度分析暂时不可用，请稍后再试。", basic: "基础模式", retry: "重新分析",
...
export default function EnglishChecker({ language = "en" }: { language?: AnalysisLanguage }) {
  ...
  async function runAnalysis() {
    ...
    setLoading(true); setStatus(ui.analysing); setUnavailable(false); setAnalysis(null);
    ...
    try {
      ...
      if (response.status === 429) { setStatus(data.error || ui.unavailable); setAnalysis(null); return; }
      if (!response.ok || data.mode === "unavailable" || !data.analysis) { setUnavailable(true); setStatus(""); return; }
      setAnalysis(data.analysis); setStatus("");
    } catch {
      if (requestId.current === currentRequest) { setAnalysis(null); setUnavailable(true); setStatus(""); }
    } finally {
  ...
  function clearConversation() {
    requestId.current += 1;
    setOtherText(""); setMyText(""); setAnalysis(null); setStatus(""); setLoading(false); setUnavailable(false); setConfirmClear(false);
  }

  function requestClear() {
    if (!otherText && !myText && !analysis && !status && !unavailable && !loading) { clearConversation(); return; }
    setConfirmClear(true);
  }

  function updateContext(next: AnalysisContext) {
    setContext(next); setAnalysis(null); setUnavailable(false); setStatus("");
  }
  ...
  return <section className="english-tool text-analyzer" id={ui.id}>
    ...
    <div className="textarea-grid">
      <label><span>{ui.other}</span><textarea value={otherText} maxLength={6000} onChange={(event) => { setOtherText(event.target.value); setAnalysis(null); setUnavailable(false); }} placeholder={ui.otherPlaceholder} /></label>
      <label><span>{ui.mine}</span><textarea value={myText} maxLength={3000} onChange={(event) => { setMyText(event.target.value); setAnalysis(null); setUnavailable(false); }} placeholder={ui.minePlaceholder} /></label>
    </div>
    ...
    <div id={`${ui.id}-result`}>
      {unavailable && <div className="analysis-unavailable"><span>{ui.basic}</span><p>{ui.unavailable}</p><button type="button" onClick={runAnalysis} disabled={!canAnalyze}>{ui.retry}</button></div>}
      {analysis && <ConversationAnalysisResult analysis={analysis} language={language} />}
    </div>
  </section>;
}
