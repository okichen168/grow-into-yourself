...
export default function SelfCheck(){
  ...
  return <section className="self-check" id="self-check">
    ...
    {!started?<div className="check-start"><div><div className="check-kind">{(Object.keys(configs) as Kind[]).map(item=><button data-context={item === "friend" ? "friendship" : item} className={kind===item?"active":""} onClick={()=>chooseKind(item)} key={item}>{kind===item&&<span aria-hidden="true">✓ </span>}{configs[item].label}</button>)}</div><strong>10道题 · 约2分钟</strong><p>选择关系后，题目会随之更换。只在当前页面计算，不上传答案、不保存分数。</p></div><button onClick={()=>setStarted(true)}>开始自查</button></div>:!showResult?<div className="question-list">
