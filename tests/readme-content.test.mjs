...
test("conversation analysis UI and rules acceptance", async () => {
  ...
  assert.match(result, /Basic analysis/); assert.match(result, /先说清楚一件事/); assert.match(result, /这段对话是怎么一步步把你带走的/); assert.doesNotMatch(result, /Evidence boundary|证据边界|Recommended reply|最推荐的回复|温和边界/);
  assert.match(guidelines, /Read the whole exchange first/); assert.match(guidelines, /Five compact calibrations/); assert.match(guidelines, /Every judgment must cite exact input text/);
  ...
  assert.match(css, /\.interaction-steps/); assert.match(css, /\.annotation-grounding/);
});
