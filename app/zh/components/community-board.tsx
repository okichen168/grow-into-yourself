...
import CommunityBarrage from "../../components/community-barrage";
...
export default function CommunityBoard({ initialLanguage = "zh" }: { initialLanguage?: "zh" | "en" }) {
  ...
  return <section className="community" id="community">
    ...
    <div className="globe-stage"><WorldGlobe posts={posts} language="zh" onSelect={setSelectedId} /><div className="globe-side"><CommunityBarrage posts={posts} language="zh" onSelect={setSelectedId} />{selected ? <div className="globe-note"><span>{[selected.city, selected.region, selected.countryName].filter(Boolean).join(" · ") || "未公开地点"}</span><blockquote>{selected.content}</blockquote><div className="note-actions"><button onClick={() => sendHeart(selected)}>♥ {copy.heart} · {selected.hearts}</button></div>{selected.replies?.length > 0 && <div className="encouragements">{selected.replies.map((item) => <p key={item.id}>✦ {item.content}</p>)}</div>}<form onSubmit={submitReply}><input value={reply} onChange={(e) => setReply(e.target.value.slice(0,120))} placeholder={copy.replyPlaceholder} /><button disabled={reply.trim().length < 2}>{copy.reply}</button>{replyNotice && <small>{replyNotice}</small>}</form></div> : null}</div><small className="globe-star-note">审核通过的留言，会在地球上变成一颗小星星。</small></div>
    <div className="community-grid single">
    ...
    <details className="feedback-disclosure"><summary>{copy.feedback}</summary><form className="soft-form feedback-form" onSubmit={submitFeedback}><p>{copy.feedbackText}</p><textarea value={feedback} onChange={(e) => setFeedback(e.target.value.slice(0,800))} placeholder={copy.feedbackPlaceholder} /><button disabled={feedback.trim().length < 4}>{copy.feedbackSubmit}</button>{feedbackNotice && <small role="status">{feedbackNotice}</small>}</form></details>
  </section>;
}
