...
export default function SelfCheck(){
  ...
  function relationAssessment(){
    if(harmScore<18)return`本次回答没有显示明显、集中的${configs[kind].label}伤害模式。一次具体伤害仍可以被认真处理。`;
    const repeated=harmScore>=40?"已经多次出现":"已经出现";
    return`${configs[kind].label}中${repeated}${strongest.label}信号。这是行为评估，不等于给对方诊断人格。`;
  }
  ...
  function supportiveReading(){
    const base={partner:"亲密不该靠害怕、监控或不断自证来维持。",family:"是家人，不等于你必须交出隐私、钱和人生选择。",workplace:"职位更高，不等于有权羞辱、孤立或故意让你失败。",friend:"友情里的泄密、排斥和背叛也会造成真正的失落。"}[kind];
  ...
  return <section className="self-check" id="self-check">
    <div className="section-heading"><p className="eyebrow">关系与身心状态自查</p><h2>不是给谁定罪，是帮你确认自己经历了什么</h2><p>{configs[kind].intro} 每种关系各有10道专门题目，结果是辅助评估，不是医疗或人格诊断。</p></div>
    {!started?<div className="check-start"><div><div className="check-kind">{(Object.keys(configs) as Kind[]).map(item=><button className={kind===item?"active":""} onClick={()=>chooseKind(item)} key={item}>{configs[item].label}</button>)}</div><strong>10道题 · 约2分钟</strong><p>选择关系后，题目会随之更换。只在当前页面计算，不上传答案、不保存分数。</p></div><button onClick={()=>setStarted(true)}>开始自查</button></div>:!showResult?<div className="question-list">
