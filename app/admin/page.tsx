...
export default function AdminPage() {
  const [key, setKey] = useState(""); const [posts, setPosts] = useState<Row[]>([]); const [feedback, setFeedback] = useState<Row[]>([]); const [error, setError] = useState("");
  async function load() { const r = await fetch("/api/admin", { headers: { "x-admin-key": key } }); const d = await r.json(); if (!r.ok) return setError(d.error || "无法进入"); setPosts(d.posts); setFeedback(d.feedback); setError(""); sessionStorage.setItem("grow-admin", key); }
  async function update(kind: "post" | "feedback", id: number, status: string) { await fetch("/api/admin", { method: "PATCH", headers: { "content-type": "application/json", "x-admin-key": key }, body: JSON.stringify({ kind, id, status }) }); await load(); }
  function download() { const blob = new Blob([JSON.stringify({ exportedAt: new Date().toISOString(), posts, feedback }, null, 2)], { type: "application/json" }); const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = `长成自己-后台导出-${new Date().toISOString().slice(0,10)}.json`; a.click(); URL.revokeObjectURL(a.href); }
